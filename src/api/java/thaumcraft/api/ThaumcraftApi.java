...
public class ThaumcraftApi {
	 ...
	 * 
	 * @param in
	 *            The input of the smelting operation. e.g. new
	 *            ItemStack(Block.oreGold)
	 * @param out
	 ...
	 * 
	 * @param in
	 *            The ore dictionary input of the smelting operation. e.g.
	 *            "oreGold"
	 * @param out
	 *            The bonus item that can be produced from the smelting
	 *            operation e.g. new ItemStack(nuggetGold,0,0). Stacksize should
	 ...
	 *            if it will work without research
	 * @param enchantment
	 *            the enchantment that will be applied to the item
	 * @param instability
	 *            a number that represents the N in 1000 chance for the infusion
	 *            altar to spawn an instability effect each second while the
	 *            crafting is in progress
	 ...
	 *            the output result
	 * @param catalyst
	 *            an itemstack of the catalyst or a string if it is an ore
	 *            dictionary item
	 * @param cost
	 *            the vis cost
	 * @param tags
	 ...
	 * <p>
	 * <i>ThaumcraftApi.registerObjectTag(new ItemStack(Blocks.cobblestone),
	 * (new AspectList()).add(Aspect.ENTROPY, 1).add(Aspect.EARTH, 1));</i>
	 * 
	 * @param item
	 *            the item passed. Pass OreDictionary.WILDCARD_VALUE if all
	 *            damage values of this item/block should have the same aspects
	 * @param aspects
	 ...
	 * declaration for cobblestone:
	 * <p>
	 * <i>ThaumcraftApi.registerObjectTag(new ItemStack(Blocks.cobblestone), new
	 * int[]{0,1}, (new AspectList()).add(Aspect.ENTROPY, 1).add(Aspect.EARTH,
	 * 1));</i>
	 * 
	 * @param item
	 * @param meta
	 ...
	 * ItemStack(Blocks.cobblestone), (new AspectList()).add(Aspect.MECHANISM,
	 * 2).add(Aspect.MOTION, 4));</i>
	 * 
	 * @param item
	 *            , pass OreDictionary.WILDCARD_VALUE to meta if all damage
	 *            values of this item/block should have the same aspects
	 * @param aspects
	 *            A ObjectTags object of the associated aspects
	 */
	 ...
	 * grown. Sending a metadata of [OreDictionary.WILDCARD_VALUE] will mean the
	 * metadata won't get checked. Example for vanilla wheat:
	 ...
	 * method. Sending a metadata of [OreDictionary.WILDCARD_VALUE] will mean
	 * the metadata won't get checked. Example (this will technically do nothing
	 * since clicking wheat does nothing, but you get the idea):
	 * FMLInterModComms.sendMessage("Thaumcraft", "harvestClickableCrop", new
	 * ItemStack(Block.crops,1,7));
	 * 
	 * Stacked crops (like reeds) are crops that you wish the bottom block
	 * should remain after harvesting. As for standard crops, you need to create
	 * and ItemStack that tells the golem what block id and metadata represents
	 * the crop when fully grown. Sending a metadata of
	 * [OreDictionary.WILDCARD_VALUE] will mean the actualy md won't get
	 * checked. If it has the order upgrade it will only harvest if the crop is
	 * more than one block high. Example:
	...
	/**
	 * You can define certain ores that will have a chance to produce native
	 * clusters via FMLInterModComms in your @Mod.Init method using the
	 * "nativeCluster" string message. The format should be:
	 * "[ore item/block id],[ore item/block metadata],[cluster item/block id],[cluster item/block metadata],[chance modifier float]"
	 * 
	 * NOTE: The chance modifier is a multiplier applied to the default chance
	 * for that cluster to be produced (default 27.5% for a pickaxe of the core)
	 * 
	 * Example for vanilla iron ore to produce one of my own native iron
	 * clusters (assuming default id's) at double the default chance:
	 * FMLInterModComms.sendMessage("Thaumcraft",
	...
	// ///////////////////////////////////////////////////////////////////////////
	/**
	 * You can blacklist crops that should not be effected by the Lamp of Growth
	 * via FMLInterModComms in your @Mod.Init method using the "lampBlacklist"
	 * itemstack message. Sending a metadata of [OreDictionary.WILDCARD_VALUE]
	 * will mean the metadata won't get checked. Example for vanilla wheat:
	 * FMLInterModComms.sendMessage("Thaumcraft", "lampBlacklist", new
	...
	// ///////////////////////////////////////////////////////////////////////////
	/**
	 * You can blacklist a dimension to not spawn certain thaumcraft features in
	 * your @Mod.Init method using the "dimensionBlacklist" string message in
	 * the format "[dimension]:[level]" The level values are as follows: [0]
	 * stop all tc spawning and generation [1] allow ore and node generation [2]
	...
	// ///////////////////////////////////////////////////////////////////////////
	/**
	 * You can blacklist a biome to not spawn certain thaumcraft features in
	 * your @Mod.Init method using the "biomeBlacklist" string message in the
	 * format "[biome id]:[level]" The level values are as follows: [0] stop all
	 * tc spawning and generation [1] allow ore and node generation [2] allow
	 * mob spawning [3] allow ore and node gen + mob spawning Example:
