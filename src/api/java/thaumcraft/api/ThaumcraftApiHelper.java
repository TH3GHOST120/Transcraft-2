...
public class ThaumcraftApiHelper {
	...
	/**
	 * Use to subtract vis from a wand for most operations Wands store vis
	 * differently so "real" vis costs need to be multiplied by 100 before
	 * calling this method
	 * 
	 * @param wand
	 *            the wand itemstack
	 ...
	 * @param doit
	 *            actually subtract the vis from the wand if true - if false
	 *            just simulate the result
	 * @param crafting
	 *            is this a crafting operation or not - if false then things
	 *            like frugal and potency will apply to the costs
	...
	public static boolean consumeVisFromWand(ItemStack wand, EntityPlayer player, AspectList cost, boolean doit, boolean crafting) {
		...
		try {
			if (consumeVisFromWand == null) {
				...
				consumeVisFromWand = fake.getMethod("consumeAllVis", ItemStack.class, EntityPlayer.class, AspectList.class, boolean.class, boolean.class);
			}
			ot = (Boolean) consumeVisFromWand.invoke(consumeVisFromWand.getDeclaringClass().cast(wand.getItem()), wand, player, cost, doit, crafting);
		}
		catch (Exception ex) {
	...
	/**
	 * Subtract vis for use by a crafting mechanic. Costs are calculated
	 * slightly differently and things like the frugal enchant is ignored Must
	 * NOT be multiplied by 100 - send the actual vis cost
	 * 
	 * @param wand
	 *            the wand itemstack
	 * @param player
	...
	public static boolean consumeVisFromWandCrafting(ItemStack wand, EntityPlayer player, AspectList cost, boolean doit) {
		...
		try {
			if (consumeVisFromWandCrafting == null) {
				Class fake = Class.forName("thaumcraft.common.items.wands.ItemWandCasting");
				consumeVisFromWandCrafting = fake.getMethod("consumeAllVisCrafting", ItemStack.class, EntityPlayer.class, AspectList.class, boolean.class);
			}
			ot = (Boolean) consumeVisFromWandCrafting.invoke(consumeVisFromWandCrafting.getDeclaringClass().cast(wand.getItem()), wand, player, cost, doit);
		}
		catch (Exception ex) {
	...
	/**
	 * Subtract vis from a wand the player is carrying. Works like
	 * consumeVisFromWand in that actual vis costs should be multiplied by 100.
	 * The costs are handled like crafting however and things like frugal don't
	 * effect them
	 * 
	 * @param player
	 *            the player using the wand
