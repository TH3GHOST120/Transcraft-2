...
public class PartEventHandler {
	...
	public void playerInteract(PlayerInteractEvent event) {
		if (event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK && event.entityPlayer.worldObj.isRemote) {
			...
			if (placing.get() != null)
				return;// for mods that do dumb stuff and call this event like
						// MFR
			placing.set(event);
	...
	public static boolean place(EntityPlayer player, World world) {
		...
		if (heldBlock == TranscraftBlocks.IronTorch)
			part = IronTorchPart.placement(world, pos, hit.sideHit);
		...
		if (!world.isRemote) {
			...
			player.swingItem();
			new PacketCustom(TcMultipartSPH.channel, 1).sendToServer();
		}
		return true;
	}
