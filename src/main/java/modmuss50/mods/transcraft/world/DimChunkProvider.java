...
public class DimChunkProvider implements IChunkProvider {
	/** RNG. */
	private Random					rand;
...

	{
		...
		villageGenerator.villageSpawnBiomes.add(Arrays.asList(new BiomeGenBase[] { TranscraftBiomes.Translands }));
		mineshaftGenerator = (MapGenMineshaft) TerrainGen.getModdedMapGen(mineshaftGenerator, MINESHAFT);
		scatteredFeatureGenerator = (MapGenScatteredFeature) TerrainGen.getModdedMapGen(scatteredFeatureGenerator, SCATTERED_FEATURE);
		ravineGenerator = TerrainGen.getModdedMapGen(ravineGenerator, RAVINE);
	}

	public DimChunkProvider(World par1World, long par2, boolean par4) {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D) {
									...
									if (k2 * 8 + l2 < b0) {
										p_147424_3_[j3 += short1] = Fluids.LiquidTransmutter.get();
									}
									else {
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		if (!flag && this.rand.nextInt(2) == 0 && TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, LAKE)) {
			k1 = k + this.rand.nextInt(16) + 8;
			l1 = this.rand.nextInt(256);
			i2 = l + this.rand.nextInt(16) + 8;
			(new WorldGenLakes(Fluids.LiquidTransmutter.get())).generate(this.worldObj, this.rand, k1, l1, i2);
		}
